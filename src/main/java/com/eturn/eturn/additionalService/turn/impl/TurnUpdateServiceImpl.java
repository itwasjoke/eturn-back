...
public class TurnUpdateServiceImpl implements TurnUpdateService {
    ...
    @Override
    public void updateTurn(TurnEditDTO turn, String username) {
     ...
     */
    private void validateUserAccess(Turn turn, User user) {
     ...
     */
    private void updateTurnData(Turn turn, TurnEditDTO turnDTO) {
     ...
     * @param turnDTO DTO с новыми данными.
     */
    private void updateNameAndDescription(Turn turn, TurnEditDTO turnDTO) {
     ...
     * @param turnDTO DTO с новыми данными.
     */
    private void updateAllowedGroups(Turn turn, TurnEditDTO turnDTO) {
        if (turnDTO.allowedGroups() != null && !turnDTO.allowedGroups().isEmpty()) {
            Set<Group> groups = new HashSet<>(turn.getAllowedGroups());
     ...
     * @param turnDTO DTO с новыми данными.
     */
    private void updateAllowedFaculties(Turn turn, TurnEditDTO turnDTO) {
        if (turnDTO.allowedFaculties() != null && !turnDTO.allowedFaculties().isEmpty()) {
            Set<Faculty> faculties = new HashSet<>(turn.getAllowedFaculties());
     ...
     * @param turnDTO DTO с новыми данными.
     */
    private void updateTimerAndPositionCount(Turn turn, TurnEditDTO turnDTO) {
        if (turnDTO.timer() != null) {
            turn.setTimer(turnDTO.timer());
        }
        if (turnDTO.positionCount() != null) {
            turn.setPositionCount(turnDTO.positionCount());
        }
    }
    ...
    private void updateTags(Turn turn) {
        String tags = turn.getName() + " " + turn.getDescription() + " " + turn.getAccessTags() + " " + turn.getCreator().getName();
        turn.setTags(tags);
    }
